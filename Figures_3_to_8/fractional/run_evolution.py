...
def create_individual(n_parameters):
    ...
    B = np.random.uniform(9, 16, 1)
    W = np.random.uniform(-12, 12, int(n_inputs*n_hidden + n_outputs*n_hidden))
...
def mutation(individual, mutation_rate=0.15):
    ...
    ##########
    if do_mutation[0]: 
        mutated_individual[0] = mutate_parameter(individual[0], 2, 0.0, 35) ## mutate neurons bias
    ##########
    for p in range(1, n_parameters):
        if do_mutation[p]: 
            mutated_individual[p] = mutate_parameter(individual[p], 2, -22, 22) ## mutate weigths
    ...
    for alpha in alphas:
            ...
            for generation in range(num_generations):
                ...
                if rank == 0:
                    ...
                    #########################
                    if (generation%40) == 4:
                        print(f"Generation {generation}, alpha {alpha} Best Fitness: {best_fitness}")
                    #########################
            ...
            if rank == 0:
                print(f'evolution {evolution};  Best fitness: {best_fitness}')
                with open('data/evolution_%.1f_%d.pkl'%(alpha, evolution), 'wb') as f:
